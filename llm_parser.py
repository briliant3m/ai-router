...
def parse_deal(
    ...
    today = _date.today()
    days_to_year_end = (_date(today.year, 12, 31) - today).days
    days_to_quarter_end = (
        _date(today.year, ((today.month - 1) // 3 + 1) * 3, 1)
        if ((today.month - 1) // 3 + 1) * 3 <= 12
        else _date(today.year, 12, 31)
    )
    ...
    if qm > 12:
        qm = 12
    import calendar
    days_to_quarter_end = (_date(today.year, qm, calendar.monthrange(today.year, qm)[1]) - today).days
    ...
    user_prompt = f"""\
СЕГОДНЯ: {today.strftime("%d.%m.%Y")} (до конца года: {days_to_year_end} дн., до конца квартала: {days_to_quarter_end} дн.)
...
- need_days: для диапазонов используй ВЕРХНЮЮ границу: "2–3 месяца" → 90, "3–4 месяца" → 120, "4–6 месяцев" → 180, "6 мес" → 180, "в теч 6 мес" → 180, "в ближайшее время" → 30, "без ограничений" → null. Для дат-ориентиров используй поле СЕГОДНЯ: "до конца года" → {days_to_year_end}, "до конца квартала" → {days_to_quarter_end}, "через месяц" → 30, "через 2 месяца" → 60 и т.п. НИКОГДА не возвращай null если срок хоть как-то указан.
